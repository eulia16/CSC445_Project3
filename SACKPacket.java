...
//ack packet
public class SACKPacket extends TFTPPacket{

    private int blockNumber;
    private byte[] ackHeader;

    private DatagramPacket packet;
    public SACKPacket(DatagramPacket passedPacket, int type, int blockNum) throws UnknownHostException {
        super(passedPacket, type);
        this.packet = passedPacket;

        ackHeader = new byte[passedPacket.getData().length + 5];
        ...
        ackHeader[4] = 0;
        System.arraycopy(passedPacket.getData(), 0, ackHeader, 5, passedPacket.getData().length);
